...
database.ref('users').once("value", function(snapshot) {
   ...
   $('.prof').on('click', function() {
      ...
      $('.shelf').empty();
      $('#shelf').html(clickedNICK+'&rsquo;s Book Collection')
      // console.log(clicked);
...
$(document).ready(function () {  // only begin once page has loaded
    ...
    $("#txtBookSearch").autocomplete({ // attach auto-complete functionality to textbox
        ...
        source: function (request, response) {
          ...
          // Diaplays the books information when clicked from auto suggestions
        select: function (event, ui) {
            ...
            addBookImage2.attr("src", "assets/images/remove_book.png");
